...
function addMessages(messages) {
    angular.forEach(messages.response, function (message) {
...
function buildMessages() {
    ...
    angular.forEach(MESSAGES, function(message) {
        if (!USERS[message.uid]) {
            USERS[message.uid] = [];
        }

        USERS[message.uid].push(message);
    });
}

function next() {
    if (nextUser == size(USERS)) {
        nextUser = 0;
    } else {
        var i = 0;

        angular.forEach(USERS, function(messagesOfUser, userId) {
            if (i == nextUser) {
...
function printMessages(uid) {
    console.log('CURRENT: ' + uid);
    console.log('Users: (' + (nextUser + 1) + '/' + size(USERS) + ')');

    angular.forEach(USERS[uid], function(m) {
        console.log((m.out == 0 ? 'in' : 'out') + ' (' + new Date(m.date * 1000).toLocaleString() + '): ' + m.body);
    });
}
